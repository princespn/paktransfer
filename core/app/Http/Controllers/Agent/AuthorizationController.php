    ...
    public function authorizeForm()
    {
        ...
        if (auth()->guard('agent')->check()) {
            ...
            if (!$user->status) {
                ...
                return view($this->activeTemplate.'agent.auth.authorization.email', compact('user', 'pageTitle'));
            }elseif (!$user->sv) {
                if (!$this->checkValidCode($user, $user->ver_code)) {
                    $user->ver_code = verificationCode(6);
                    $user->ver_code_send_at = Carbon::now();
                ...
                return view($this->activeTemplate.'agent.auth.authorization.sms', compact('user', 'pageTitle'));
            }elseif (!$user->tv) {
                $pageTitle = 'Google Authenticator';
                return view($this->activeTemplate.'agent.auth.authorization.2fa', compact('user', 'pageTitle'));
            }else{
