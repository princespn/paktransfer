...
class PerfectMoney extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'perfect:money';
        ...
        if($withdrawMethod) {
            ...
            if(
                ...
                if($wallets['status'] == 'success') {
                    ...
                    $withdraws = Withdrawal::with('method')->where('status', 2)->where('perfect_money', 1)->get()->take(10);
                    foreach ($withdraws as $withdraw) {
                        $finalAmount = (float)$withdraw->final_amount;
                        if($finalAmount < $balance && isset($withdraw->withdraw_information->pm_wallet->field_name) && !empty($withdraw->withdraw_information->pm_wallet->field_name)) {
                            $finalAmount = number_format($finalAmount,2);
                            $response = $this->transfer($setting['perfect_account_id'],$setting['perfect_passphrase'],$setting['perfect_wallet'],$withdraw->withdraw_information->pm_wallet->field_name, $finalAmount, $withdraw->trx);
                            var_dump($response);
                            if(!isset($response['error']) && isset($response['message']) && isset($response['message']['PAYMENT_BATCH_NUM']) && !empty($response['message']['PAYMENT_BATCH_NUM'])){
                                $withdraw->status = 1;
                                $withdraw->admin_feedback = 'Payment Batch Num: '.$response['message']['PAYMENT_BATCH_NUM'];
                                $withdraw->save();
                            }
                        }
                    }
